//Trabajo Paracaídista Física Computacional 1
...
double euler_mejorado(double t_f,double N){
    ...
    while (t<=t_f)
    {
    double v=v_0+h*EDO(v_0);
    double v_next=v+0.5*h*(EDO(v_0)+EDO(v+h)); // definicion de variables
    v=v_next;
...
int main(){
    ...
    dataFile.close();
    //EULER MEJORADO ENTRADA Y SALIDA
    ...
    double vf=euler_mejorado(t_f,N);
...
dataFile4.close();
    ...
    scriptgp4.close();
        ...
        std::ofstream scriptgp5("euler_m.gp");  

        scriptgp5 <<"set term png \n";
        scriptgp5  <<"set output 'euler_m.png' \n";
        scriptgp5  <<"set xrange [0:40] \n";
        scriptgp5  <<"set yrange [0:-40] \n";
        scriptgp5  <<"set xlabel 'Tiempo(s)'\n";
        scriptgp5  <<"set ylabel 'Velocidad(m/s)'\n";
        scriptgp5  <<"plot 'euler_m.dat' u 1:2 w l title 'Solucion por Metodo de Euler mejorado'";
