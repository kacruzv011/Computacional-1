...
//euler vitaminas
std::ofstream dataFile4("euler_mejorado.dat");
double euler_mejorado(double t_f, double N){
    ...
    double v_asterisco=y+h*caida(v);
    double v_next=y+0.5*h*(caida(v)+caida(v_asterisco)); // definicion de variables
    v=v_next;
    t+=h;
    dataFile4 <<t<<" "<<v<<std::endl;
    }
    dataFile4.close();
    return v;
}
